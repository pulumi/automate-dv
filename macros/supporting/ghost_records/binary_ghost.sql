...
{%- macro default__create_ghost_record(
    src_pk,
    src_hashdiff,
    src_payload,
    src_extra_columns,
    ...
    {%- set src_hashdiff_name = src_hashdiff["source_column"] | default(
        src_hashdiff
    ) -%}
    {%- set col_definitions = [] -%}

    {%- set binary_columns = (
        automate_dv.expand_column_list([src_pk, src_hashdiff_name])
        | map("lower")
        | list
    ) -%}
    {%- set null_columns = (
        automate_dv.expand_column_list([src_payload, src_extra_columns])
        | map("lower")
        | list
    ) -%}
    {%- set time_columns = (
        automate_dv.expand_column_list([src_ldts, src_eff])
        | map("lower")
        | list
    ) -%}
    {%- set src_pk_lower = src_pk | lower -%}

    {%- set all_columns = (
        automate_dv.expand_column_list(
            [
                src_pk,
                src_hashdiff_name,
                src_payload,
    ...
    {%- for col in filtered_source_columns -%}
        ...
        {%- if col_compare in binary_columns -%}
            {%- do col_definitions.append(
                automate_dv.binary_ghost(alias=col_name, hash=hash)
            ) -%}
        ...
        {%- elif col_compare in null_columns -%}
            {%- do col_definitions.append(
                automate_dv.null_ghost(data_type=col_type, alias=col_name)
            ) -%}
        {# Handle anything else as its correct ghost representation #}
        {%- else -%}
            {%- do col_definitions.append(
                automate_dv.ghost_for_type(col_type, col_name)
            ) -%}
        {%- endif -%}
    ...
    select
        {% for column_def in col_definitions -%}
            {{ column_def }} {%- if not loop.last -%}, {% endif %}
        {%- endfor -%}
